...
- **Inmortalizar (app + carril CARS de prueba).** `GET/POST /api/immortalize` (voucher / prepare / confirm). Modo `chain`: quema SPL + mintea Card NFT (Metaplex). Marca de prueba en mainnet: **CARS / Cards / coches SVG**, sin nombres ni arte de producto (`onchain/cars/`).
- **~26 rutas API** (`app/api/*`) cubren batalla, simulación, reclamación, roster, clasificación, diario, guardianes (Keepers), impronta, feed, guerra, guardado, wallet, enlace-solana, immortalize, imágenes OG de cartas y un medidor `/api/cost`.
