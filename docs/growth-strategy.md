...
**Thesis:** Zingers' unfair advantage is **flight as the face** plus the **raise → fight → evolve** attachment loop. Competitors have arenas (Agent Arena, LMArena, Kaggle Game Arena); none have *your* climb *and* *your* champion — a persistent, evolving character with a career-shaped body. The strategy: make the Climb irresistible in zero seconds, make champions maximally watchable and shareable, and let attachment do the retention. Door: [two-doors.md](./two-doors.md).

---

## 1. Where we stand

**Already built and on-strategy** (see [TWOPAGER.md](./TWOPAGER.md) §3): Flight-First Climb/Circuit (100-sector Ascent), mobile `/m` Take flight door, Daily Tribunal, OG share cards, challenge links, async league with ELO, BYO-agent protocol with heuristic fallback, career-derived bodies + Saga/Imprints, unique Trainer names (+ optional wallet).
...
| No live spectating of league fights (replays only) | Spectator drama is the proven growth channel for agent content. |
...
| Climb feel playtests unfinished | Growth push waits on Flight-First gates 1′–3′ ([flight-first-plan.md](./flight-first-plan.md)). |
...
2. **Predict-the-winner everywhere, free.** Extend the Tribunal "call it before you watch" mechanic to all Amphitheatre league fights. Points, not money (Polymarket runs $34M of AI-outcome markets; we capture the impulse legally). Prediction gives spectators skin in the game — the missing agency that killed pure-spectacle formats.
...
2. **BYO-agent ranked ladder as cost strategy.** Ranked BYO agents pay their own inference. House-brain casual fights route to cheap models; frontier models reserved for tournaments (Death by AI reached profitability at 700K DAU on exactly this tiering).
...
1. **Moderation layer before any growth push.** Filter judge/champion output server-side (cheap classifier pass on lines before SSE emit); kill-switch per fight. Launch-blocking, not optional.
...
3. **Latency & cost budget per fight.** Keep turn cadence tuned for watchability (Kaggle Arena deliberately paced for spectating). Continue mock-mode-first dev; instrument per-fight token spend. Climb-first sessions should stay near-zero LLM cost.
4. **Zero-friction entry.** Keyless demo already runs on the heuristic fallback — preserve this ruthlessly. First **Take flight / Climb** (and first fight watched) must require no signup, no key, no download.

### 3.5 Art & lore (P1)

1. **Lore as shareable canon.** The Five Forces, Keepers, and Trainer saga exist in `docs/bible` and `lib/lore/` — surface them in-product: force emblems on cards, Keeper voice lines in clips, a public `/bible` codex page (route exists). Ownable IP is the stated moat.
...
**North-star metric:** share-artifact CTR → first Take flight / Climb → guest→claim wingmate → first fight watched. Instrument each hop before the growth push (`/stats` MOBILE DOOR; `docs/two-doors.md`).
