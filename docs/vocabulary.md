...
**the Grounds, the Concord, the Long Vault, the Hum, the Keepers, Force, Sigil, Saga, Resolve, Tribunal, Gauntlet, Circuit, Fragment, Crowns.** These are signature and worth keeping — but the first time each appears in any surface, add a plain gloss. The canonical one-line definitions live in `lib/lore/glossary.ts` (rendered at `/glossary` and in `docs/bible/09-glossary.md`); reuse that wording so glosses stay consistent.
