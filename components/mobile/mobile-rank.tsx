  ...
  const challenge = useCallback(
    async (c: LadderChampion) => {
      ...
      try {
        ...
        if (res.error) flash(`${t("fightFailed")} ${res.error}`);
        else flash(`${res.result.winner} · ${res.result.loser} (+${res.result.delta})`);
    ...
    <div style={{ height: "100%", overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "16px 14px 24px" }}>
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 20, fontWeight: 800 }}>{t("rankTitle")}</span>
          {!shared && (
            <span className="mono" style={{ marginLeft: "auto", fontSize: 9, color: "var(--gold)", border: "1px solid rgba(240,169,58,.4)", borderRadius: 6, padding: "3px 7px" }}>{t("localMode")}</span>
          )}
        </div>
        ...
        {mine && (
          <div className="panel" style={{ ["--ac" as string]: TYPE_COLOR[mine.type], padding: 14, marginBottom: 14, display: "flex", alignItems: "center", gap: 14, boxShadow: `0 0 26px -12px ${TYPE_COLOR[mine.type]}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="mono" style={{ fontSize: 9, letterSpacing: 1.2, color: "var(--muted2)" }}>{t("yourChampion")}</div>
              <div style={{ fontSize: 17, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{mine.name}</div>
              <div className="mono" style={{ fontSize: 10, color: TYPE_COLOR[mine.type] }}>{mine.rating} · {mine.wins}W·{mine.losses}L</div>
            </div>
            <button
              type="button"
            ...
            >
              <Swords size={14} strokeWidth={2.4} /> {busy === mine.id ? "…" : t("fight")}
            </button>
          </div>
        )}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {isMine && <span className="mono" style={{ fontSize: 7.5, color: col, border: `1px solid ${col}`, borderRadius: 4, padding: "1px 4px", flexShrink: 0 }}>{t("yours")}</span>}
                  </div>
                  <div className="mono" style={{ fontSize: 9.5, color: "var(--muted)", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {forceName(c.type)} · {c.house ? t("league") : c.key}
        ...
        <div className="panel" style={{ ["--ac" as string]: "var(--good)", padding: 14, marginTop: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            ...
            <span className="mono" style={{ fontSize: 10, letterSpacing: 1.5, color: "var(--good)" }}>{t("liveFights")}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
